...
const Profile = ({ navigation, route }: Props) => {
    ...
    useLayoutEffect(() => {
        navigation.setOptions({
            ...
            headerRight: () => (
                <Avatar url={user?.avatarUrl} size={38} editable={false} rounded={true} margin={5} />
            ),
            headerTitle: () => (
                <Animated.Text style={[styles.title, animatedHeaderStyles]}>{user?.name || ''}</Animated.Text>
            ),
        })
    }, [navigation, user])
    ...
    if (loading || !user || !fontsLoaded) {
        return (
            <>
                <Text>
                    {`Loading... ${loading} and ${JSON.stringify(user)}`}
                </Text>
            </>
        )
    }
    ...
    return (
        <View style={styles.container}>
            ...
            >
                ...
                <Animated.Text style={[styles.usernameHeader, animatedUsernameStyles]}>{user.name}</Animated.Text>
                <Socials user={user}/>
